...
class SoftMax(Activation):
    ...
    def forward(self, Z: np.ndarray) -> np.ndarray:
        ...
        ### YOUR CODE HERE ###
        # Z = si
        si_m = Z - np.max(Z, axis= -1, keepdims=True) # axis = -1 max of every most inner list 
        e_si_m = np.exp(si_m) #e^(si-m)
        out = e_si_m / (np.sum(e_si_m, axis=-1, keepdims=True))
        return out
    ...
    def backward(self, Z: np.ndarray, dY: np.ndarray) -> np.ndarray:
        ...
        """
        ### YOUR CODE HERE ###
        # calc dY/dZ (jacobian) then np.dot(dL/dY, dY/dZ)
        Y = self.forward(Z)
        backward = []
        for i, sample in enumerate(Y): 
            diag = np.diagflat(sample)
            sample = sample.reshape((-1, 1)) # nx1 -> np.array([[0.1], [0.5], [0.2], [0.2]]) column vector 
            jacob = diag - np.dot(sample, sample.T) # outer product nxn; diag - sigma ^2 = sigma (1 - sigma)
            backward.append(dY[i] @ jacob)
